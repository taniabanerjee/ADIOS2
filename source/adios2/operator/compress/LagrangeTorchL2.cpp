...
void LagrangeTorchL2::computeParamsAndQoIs(const std::string meshFile,
...
void LagrangeTorchL2::getUnconvergedIndexes(at::Tensor &diff, std::vector<long>& unconvergedNodeIndex, std::map<long, long> &unconvergedMap)
{
...
int LagrangeTorchL2::lambdaIterationsRound(int maxIter, double stepsize, at::Tensor &lambdas_torch, std::vector<long>& unconvergedNodeIndex, int nodes, at::Tensor &recon_torch, at::Tensor &orig_torch, at::Tensor &v_torch, at::Tensor &v2_torch, at::Tensor &v3_torch, at::Tensor &v4_torch, at::Tensor &d_torch, at::Tensor &u_torch, at::Tensor &t_torch, at::Tensor &r_torch, double DeB, double UeB, double TperpEB, double TparaEB, double PDeB)
{
    auto aD_torch = d_torch*mySmallElectronCharge;
    auto gradients_torch = torch::zeros({nodes,4}, ourGPUOptions);
    auto hessians_torch = torch::zeros({nodes,4,4}, ourGPUOptions);
    auto L2_den = torch::zeros({nodes, maxIter+1}, ourGPUOptions);
    auto L2_upara = torch::zeros({nodes, maxIter+1}, ourGPUOptions);
    auto L2_tperp = torch::zeros({nodes, maxIter+1}, ourGPUOptions);
    auto L2_rpara = torch::zeros({nodes, maxIter+1}, ourGPUOptions);
    auto L2_pd = torch::zeros({nodes, maxIter+1}, ourGPUOptions);
...
// Custom Dataset class for Post Processing (PP)
class PPDataset : public torch::data::datasets::Dataset<PPDataset>
{
};
    ...
    const double* reconData, adios2::Dims blockCount)
{
    ...
    compareQoIs(recondatain, outputs);
        ...
        myfile << "Data dimensions " << combined.sizes() << std::endl;
        myfile << "Start index: Plane " << myPlaneOffset << " Node " << myNodeOffset << " Vx " << 0 << " Vy " << 0 << std::endl;
        myfile << "Total counts: Plane " << myPlaneCount << " Node " << myNodeCount << " Vx " << myVxCount << " Vy " << myVyCount << std::endl;
...
void LagrangeTorchL2::readF0Params(const std::string meshFile)
{
...
void LagrangeTorchL2::setVolume()
{
#if 0
...
void LagrangeTorchL2::setVp()
{
    for (int ii = -myF0Nvp[0]; ii<myF0Nvp[0]+1; ++ii) {
        myVp.push_back(ii*myF0Dvp[0]);
    }
    myVpTorch = torch::from_blob((void *)myVp.data(), {myVyCount}, torch::kFloat64).to(torch::kCUDA);
    return;
}

void LagrangeTorchL2::setMuQoi()
{
...
void LagrangeTorchL2::compute_C_qois(int iphi, at::Tensor &density, at::Tensor &upara, at::Tensor &tperp, at::Tensor &tpara, at::Tensor &n0, at::Tensor &t0, at::Tensor &dataInTorch)
{
...
void LagrangeTorchL2::compareQoIs(at::Tensor& reconData, at::Tensor& bregData)
{
    int iphi = 0;
    int my_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
...
void LagrangeTorchL2::compareErrorsPD(at::Tensor& dataIn, at::Tensor& reconData, at::Tensor& bregData, const char* etype, int rank)
{
