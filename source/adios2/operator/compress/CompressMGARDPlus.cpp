                                  ...
                                  const DataType type, char *bufferOut)
{
    ...
    else if (compression_method == 4)
    {
        ...
        *reinterpret_cast<int *>(bufferOut) = latent_dim;
        ...
        MPI_Allreduce(&diff_cnt, &diff_allcnt, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        if (my_rank == 0) 
        {
            printf("Error min,max,mse: %g %g %g\n", diff_allmin, diff_allmax, diff2_allsum/diff_allcnt);
        }
